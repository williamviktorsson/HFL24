...
class ItemsBloc extends Bloc<ItemsEvent, ItemsState> {
  ...
  Future<void> onDeleteItem(Emitter<ItemsState> emit, Item item) async {
    ...
    final items = await repository.getAll();
    emit(ItemsLoaded(
        items: items, feedback: ["Item ${item.description} deleted"]));
  }
  ...
  Future<void> onCreateItem(Item item, Emitter<ItemsState> emit) async {
    ...
    emit(ItemsLoaded(
        items: items, feedback: ["Item ${item.description} created"]));
  }
  ...
  Future<void> onUpdateItem(Item item, Emitter<ItemsState> emit) async {
    ...
    final items = await repository.getAll();
    emit(ItemsLoaded(
        items: items, feedback: ["Item ${item.description} updated"]));
  }
