...
import 'package:admin_app/cubit/filter_items_cubit.dart';
    ...
    return Scaffold(
      body: Center(
        child: RefreshIndicator(
          onRefresh: () async {
            context.read<ItemsBloc>().add(ReloadItems());
          ...
          child: BlocConsumer<ItemsBloc, ItemsState>(
            listenWhen: (previous, current) => current is ItemsLoaded && current.feedback.isNotEmpty,            
            listener: (context, state) {
              if (state is ItemsLoaded && state.feedback.isNotEmpty) {
                ScaffoldMessenger.of(context)
                    .showSnackBar(SnackBar(content: Text(state.feedback[0])));
              }
            },
            builder: (context, itemsState) {
              return switch (itemsState) {
                ItemsInitial() => TextButton(
                    onPressed: () {
                  ...
                  Builder(builder: (context) {
                    return Column(
                      children: [
                        TextField(
                          onChanged: (text) {
                            context.read<FilterItemsCubit>().filter(text);
                          },
                        ),
                        Divider(),
                        Expanded(
                          child: BlocBuilder<FilterItemsCubit, String>(
                              builder: (context, term) {
                            final filteredItems = items
                                .where(
                                    (item) => item.description.contains(term))
                                .toList();

                            return BlocBuilder<SelectedItemCubit, Item?>(
