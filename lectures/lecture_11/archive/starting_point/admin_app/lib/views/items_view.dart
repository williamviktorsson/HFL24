    ...
    return Scaffold(
        ...
        child: BlocBuilder<ItemsBloc, ItemsState>(
          builder: (context, state) {
            return switch (state) {
              ...
              ItemSyncing(items: var items) =>
                RefreshIndicator(
                  onRefresh: () async {
                    context.read<ItemsBloc>().add(const RefreshItems());
                          ...
                          child: Material(
                            child: BlocBuilder<ItemsBloc, ItemsState>(
                                buildWhen: (previous, current) =>
                                    current is ItemSyncing &&
                                    current.changedItem?.id == item.id,
                                builder: (context, itemsState) {
                                  return BlocBuilder<SelectedItemCubit, Item?>(
                                      builder: (context, state) {
                                    bool selected = state?.id == item.id;

                                    bool syncing = itemsState is ItemSyncing &&
              ...
              ItemsError(message: var message) => Text('Error: ${message}'),
            };
          },
        ),
      ),
      ...
      floatingActionButton: AnimatedActionButtons(
        ...
        onItemEdited: (item) async {
          context.read<ItemsBloc>().add(UpdateItem(item));
        },
        onItemDeleted: (item) async {
          context.read<ItemsBloc>().add(DeleteItem(item));
          context.read<SelectedItemCubit>().deselect();
        },
      ),
    );
